                            ...
                            <table id="basic-datatables" class="display table table-striped table-hover" >
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge badge-primary"><b>{{ $res->barangMasuk->no_nota }}</b></span></td>
                                        <td>{{ $res->barang->nama_barang }}</td>
                                        <td>{{ $res->jumlah }}</td>
                                        <td>{{ $res->sub_total }}</td>
                                        <td>
                                            <a id="{{ $res->id }}" href="#" class="edit btn btn-warning btn-sm"><i class="far fa-edit"></i></a>
                                            <a data-toggle="modal" data-target="#konfirmasi_hapus" data-href="{{ route('detailBarangDelete', $res->id) }}" class="btn btn-danger btn-sm"><span style="color:#fff;"><i class="far fa-trash-alt"></i></span></a>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('detailBarangSave') }}" enctype="multipart/form-data">
                @csrf
                    <div class="row">
                        <div class="col-12 col-lg-6">
                            <div class="form-group">
                                <label>Nota Barang Masuk</label>
                                <select name="id_barang_masuk" class="form-control select2" required="">
...
<div class="modal fade" id="addRowModal2" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="FormEditDetailBarang" method="POST" action="" enctype="multipart/form-data">
                @csrf
                <div class="row">
                        <div class="col-12 col-lg-6">
                            <div class="form-group">
                    ...
                    <div class="col-12 col-lg-6">
                        <div class="form-group">
                            <label>Jumlah</label>
                            <input onkeypress="return isNumberKey(event)" type="text" class="form-control" name="jumlah" id="jumlah" required="">
                        </div>
                    </div>
                    <div class="col-12 col-lg-6">
                        <div class="form-group">
                            <label>Sub Total</label>
                            <input onkeypress="return isNumberKey(event)" type="text" class="form-control" name="sub_total" id="sub_total" required="">
                        </div>
                    </div>
                </div>
                    <div class="modal-footer no-bd">
		...
		$(document).on('click', '.edit', function(e){
			...
			var id = $(this).attr('id');
			$.ajax({
				url: "{{ url('barang/detailBarang/getDataEdit') }}/"+id,
				dataType: "JSON",
				success:function(data){
					if(data != ""){
						...
						$('#openModalEdit').click();
						$('#FormEditDetailBarang').attr("action", "{{ url('barang/detailBarang/prosesEdit') }}/"+id);
					}
				}
			})
		})
